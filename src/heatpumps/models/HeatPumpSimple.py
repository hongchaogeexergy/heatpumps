...
import numpy as np
import pandas as pd
# Import new TESPy components and connections for power flows
from tespy.components import (
    Compressor, Condenser, CycleCloser, HeatExchanger, Pump,
    Sink, Source, Valve,
    PowerSource, PowerBus, Motor  # added for TESPy 0.9 power flow
)
...
class HeatPumpSimple(HeatPumpBase):
    ...
    def generate_components(self):
        ...
        # Power supply components for TESPy 0.9 (replace Bus):
        self.comps['power_source'] = PowerSource('Power Source')
        self.comps['power_bus'] = PowerBus('Power Distribution', num_in=1, num_out=3)
        self.comps['motor_comp'] = Motor('Motor Compressor')
        self.comps['motor_hs'] = Motor('Motor HS Pump')
        self.comps['motor_cons'] = Motor('Motor Cons Pump')
    ...
    def generate_connections(self):
        ...
        # e3: Mechanical shaft power from consumer pump motor to heat sink pump
        self.conns['e3'] = PowerConnection(
    ...
    def init_simulation(self, **kwargs):
        ...
        # Components
        # If you want to skip the initial enthalpy guess on A4, keep this commented:
        # self.conns['A4'].set_attr(h=Ref(self.conns['A3'], self._init_vals['dh_rel_comp'], 0))

        # IMPORTANT: apply compressor efficiency already in init (not only in design).
        _eta_comp = self._normalize_eta(self.params['comp']['eta_s'])
        self.comps['comp'].set_attr(eta_s=_eta_comp)

        self.comps['hs_pump'].set_attr(eta_s=self._normalize_eta(self.params['hs_pump']['eta_s']))
        hsink_params = self.params.get('hsink_pump', self.params.get('cons_pump', {}))
        if 'eta_s' in hsink_params:
            self.comps['hsink_pump'].set_attr(eta_s=self._normalize_eta(hsink_params['eta_s']))
        self.comps['evap'].set_attr(pr1=self.params['evap']['pr1'], pr2=self.params['evap']['pr2'])
        self.comps['cond'].set_attr(pr1=self.params['cond']['pr1'], pr2=self.params['cond']['pr2'])

        # Connections – starting values
        t_cond = self.params.get('C2', {}).get('T', self.params['C3']['T'])
        p_evap, p_cond, _ = self.get_pressure_levels(
            T_evap=self.params['B2']['T'], T_cond=t_cond
        )
        ...
        # If you uncommented the A4 enthalpy guess above, clear it here:
        # self.conns['A4'].set_attr(h=None)

        print("DEBUG Compressor params:",
              self.params['comp']['eta_s'],
              self.conns['A3'].T.val, self.conns['A3'].p.val, self.conns['A4'].p.val)
        print("DEBUG Compressor eta_s used in INIT (normalized):", _eta_comp)

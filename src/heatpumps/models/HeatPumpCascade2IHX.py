...
class HeatPumpCascade2IHX(HeatPumpCascadeBase):
    ...
    def init_simulation(self, **kwargs):
        ...
        self.comps['ihx1'].set_attr(
            pr1=self.params['ihx1']['pr1'], pr2=self.params['ihx1']['pr2']
            )
        self.comps['ihx2'].set_attr(
            pr1=self.params['ihx2']['pr1'], pr2=self.params['ihx2']['pr2']
            )

        # Connections
        # Starting values
        t_sink_hot = self.get_sink_hot_design_temperature()
        t_source_cold = self.get_source_cold_design_temperature()
        self.T_mid = self.get_design_t_mid(t_source_cold, t_sink_hot)
        ...
        h_superheat1 = PSI(
            'H', 'P', p_evap1*1e5,
            'T', (
                t_source_cold - self.params['evap']['ttd_l'] + 273.15

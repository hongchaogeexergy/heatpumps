...
class HeatPumpCascadeIHXPC(HeatPumpCascadeBase):
    """Two stage cascading heat pump with two refrigerants and open or closed economizer,
    ...
    def generate_components(self):
        ...
        self.comps['cc2'] = CycleCloser('Upper Main Cycle Closer 2')
        ...
        # Main lower cycle
        self.comps['cc1'] = CycleCloser('Lower Main Cycle Closer 1')
    ...
    def generate_connections(self):
        ...
        self.conns['A4'] = Connection(
            self.comps['econ2'], 'out1', self.comps['evap_valve2'], 'in1', 'A4'
        )
        ...
        self.conns['A9'] = Connection(
            self.comps['merge2'], 'out1', self.comps['cond'], 'in1', 'A9'
        )
        self.conns['A10'] = Connection(
            self.comps['econ2'], 'out2', self.comps['HT_comp2'], 'in1', 'A10'
        )
        self.conns['A11'] = Connection(
            self.comps['HT_comp2'], 'out1', self.comps['merge2'], 'in2', 'A11'
        )

        if self.econ_type.lower() == 'closed':
            self.conns['A2'] = Connection(
                self.comps['ihx2'], 'out1', self.comps['split2'], 'in1', 'A2'
            )
            self.conns['A3'] = Connection(
                self.comps['split2'], 'out1', self.comps['econ2'], 'in1', 'A3'
            )
            self.conns['A12'] = Connection(
                self.comps['split2'], 'out2', self.comps['mid_valve2'], 'in1', 'A12'
            )
            self.conns['A13'] = Connection(
                self.comps['mid_valve2'], 'out1', self.comps['econ2'], 'in2', 'A13'
            )
        elif self.econ_type.lower() == 'open':
            self.conns['A2'] = Connection(
                self.comps['ihx2'], 'out1', self.comps['mid_valve2'], 'in1', 'A2'
            )
            self.conns['A3'] = Connection(
        ...
        self.conns['D4'] = Connection(
            self.comps['econ1'], 'out1', self.comps['evap_valve1'], 'in1', 'D4'
        )
        ...
        self.conns['D9'] = Connection(
            self.comps['merge1'], 'out1', self.comps['inter'], 'in1', 'D9'
        )
        self.conns['D10'] = Connection(
            self.comps['econ1'], 'out2', self.comps['LT_comp2'], 'in1', 'D10'
        )
        self.conns['D11'] = Connection(
            self.comps['LT_comp2'], 'out1', self.comps['merge1'], 'in2', 'D11'
        )

        if self.econ_type.lower() == 'closed':
            self.conns['D2'] = Connection(
                self.comps['ihx1'], 'out1', self.comps['split1'], 'in1', 'D2'
            )
            self.conns['D3'] = Connection(
                self.comps['split1'], 'out1', self.comps['econ1'], 'in1', 'D3'
            )
            self.conns['D12'] = Connection(
                self.comps['split1'], 'out2', self.comps['mid_valve1'], 'in1', 'D12'
            )
            self.conns['D13'] = Connection(
                self.comps['mid_valve1'], 'out1', self.comps['econ1'], 'in2', 'D13'
            )
        elif self.econ_type.lower() == 'open':
            self.conns['D2'] = Connection(
                self.comps['ihx1'], 'out1', self.comps['mid_valve1'], 'in1', 'D2'
            )
            self.conns['D3'] = Connection(
    ...
    def init_simulation(self, **kwargs):
        ...
        self.conns['D11'].set_attr(
            h=Ref(self.conns['D10'], self._init_vals['dh_rel_comp'], 0)
            )
        ...
        self.conns['D11'].set_attr(h=None)

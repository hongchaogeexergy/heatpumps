...
class HeatPumpEconIHXTrans(HeatPumpBase):
    ...
    def generate_connections(self):
        ...
        self.conns['A9'] = Connection(
            self.comps['merge'], 'out1', self.comps['comp2'], 'in1', 'A9'
            )
        self.conns['A10'] = Connection(
            self.comps['comp2'], 'out1', self.comps['trans'], 'in1', 'A10'
            )
        self.conns['A11'] = Connection(
            self.comps['econ'], 'out2', self.comps['merge'], 'in2', 'A11'
            )
    ...
    def init_simulation(self, **kwargs):
        ...
        self.conns['A0'].set_attr(p=self.params['A0']['p'], h=h_trans_out, fluid={self.wf: 1})
        if t_sink_hot == 90:
        ...
        if self.econ_type == 'open':
            ...
            h_mix_mid = PSI(
                ...
                'T', (t_sink_hot + self.params['trans']['ttd_l']
